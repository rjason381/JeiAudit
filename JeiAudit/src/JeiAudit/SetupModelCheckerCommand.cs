...
namespace JeiAudit
{
    ...
    public class SetupModelCheckerCommand : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                using (var form = new ModelCheckerSetupForm(defaultPath))
                {
            ...
            {
                message = ex.Message;
                TaskDialog.Show("JeiAudit", $"Setup failed.{Environment.NewLine}{ex.Message}");
